...
class DataLoader:
    ...
    def load_data(noise_level: str, chaotic: bool = True, uniform: bool = True) -> Tuple[torch.Tensor, ...]:
        ...
        def load_and_process(prefix: str) -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            # 여기서는 vel과 acc만 사용: 
            return sorted_data[:, 0], sorted_data[:, [1,2,3,4,5,6,7,8,9]]
...
def main():
    ...
    best_model_path = f"{config['save_dir']}/trained_models/MBD_VT_model_pos_vel_acc_{noise_level_str}.pth"
    ...
    if training_mode:
        ...
        results_fig_path = f"{config['save_dir']}/figs/MBD_VT_model_pos_vel_acc_{noise_level_str}.png"
