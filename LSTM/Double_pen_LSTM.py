...
def preprocess_data(time_data, data):
    ...
    # sorted_data의 컬럼: [time, angle1, angular_velocity1, angular_acceleration1, angle2, angular_velocity2, angular_acceleration2]
...
class LSTMPendulum(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=2):
        """
        Args:
            input_size: 입력 차원 (여기서는 시간 t만 사용 → 1)
            hidden_size: LSTM 은닉층 크기
            output_size: 출력 차원 (두 진자의 각가속도 → 2)
...
def train_model(model, criterion, optimizer, scheduler,
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss.item() < best_valid_loss:
            ...
            best_model_path = f'example/trained_models/double_pen_LSTM_model_{noise_level}.pth'
...
def plot_results(time_test: np.ndarray, test_data: np.ndarray, predictions: np.ndarray, 
    ...
    """
    결과 시각화 및 저장
    test_data와 predictions는 (n,2) 형태로 되어 있으며,
      - 첫 번째 열: Pendulum 1 Angular Acceleration
      - 두 번째 열: Pendulum 2 Angular Acceleration
    """
    plt.figure(figsize=(15, 5))
    
    plt.plot(time_test[seq_length:], test_data[seq_length:, 0], label='True P1 Angular Acceleration', color='black', linestyle='-')
    plt.plot(time_test[seq_length:], predictions[:, 0], label='Predicted P1 Angular Acceleration', color='red', linestyle='--')
    
    plt.plot(time_test[seq_length:], test_data[seq_length:, 1], label='True P2 Angular Acceleration', color='black', linestyle='-')
    plt.plot(time_test[seq_length:], predictions[:, 1], label='Predicted P2 Angular Acceleration', color='blue', linestyle='--')
...
def main():
    ...
    seq_length = 20
    hidden_size = 50
    output_size = 2         # 두 진자의 각가속도 예측 → 2
    num_layers = 2
    num_epochs = 200
    learning_rate = 0.001
    batch_size = 32

    # 데이터 로드 및 전처리 (base path: data/double_pendulum)
    ...
    # 결과 플롯 및 저장
    results_fig_path = f'example/figs/double_pen_LSTM_fig_{noise_level}.png'
