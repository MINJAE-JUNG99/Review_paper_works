...
class DataLoader:
    ...
    def load_data(self, noise_level: str, damping: bool = False) -> Tuple[np.ndarray, ...]:
        """데이터 로드 및 전처리"""
        try:
            folder = "single_pendulum_damped" if damping else "single_pendulum_no_damped"
    ...
    def _preprocess_data(self, datasets: dict) -> Tuple[torch.Tensor, ...]:
        ...
        여기서는 원본 파일에 저장된 출력이 
        [angle, angular_velocity, angular_acceleration] 순으로 되어 있으므로,
        오직 각가속도 (angular_acceleration)만 선택합니다.
        """
        processed_data = {}
        ...
        for split in ['train', 'test', 'valid']:
            ...
            # 시간과 출력 데이터 분리
            # sorted_data의 컬럼 구성: [time, angle, angular_velocity, angular_acceleration]
            processed_data[f"time_{split}"] = torch.FloatTensor(sorted_data[:, 0]).unsqueeze(1)
            # 오직 각가속도(인덱스 3)만 사용하여 (n,1) 형태로 변환
            processed_data[f"data_{split}"] = torch.FloatTensor(sorted_data[:, 3:4])
...
def plot_results(time_test: np.ndarray, test_data: np.ndarray, predictions: np.ndarray, 
    ...
    # true와 predicted 모두 오직 각가속도만 있으므로 단일 곡선 플롯
    plt.plot(time_test, test_data[:, 0], label='True Angular Acceleration', 
             color='black', linestyle='-')
    plt.plot(time_test, predictions[:, 0], label='Predicted Angular Acceleration', 
             color='red', linestyle='--')
    
    # Extrapolation 구간 시작 시간 (중앙값)
...
def main():
    ...
    config = {
        ...
        'hidden_sizes': [50, 50],
        'output_size': 1,           # 출력: 오직 각가속도만 예측 (1)
        'num_epochs': 200,
        'learning_rate': 0.001,
        'noise_level': 'clean',     # 예: 'clean', 'noise_10%', 'noise_30%' 등
        'damping': True
    }
    ...
    for epoch in range(config['num_epochs']):
        ...
        if valid_loss < best_valid_loss:
            ...
            patience_counter = 0
            # 최상의 모델 저장
            torch.save(model.state_dict(), f'example/trained_models/single_pen_FFN_best_model_{config["noise_level"]}.pth')
        else:
            patience_counter += 1
    ...
    plot_results(time_test.numpy(), data_test.numpy(), predictions, 
                 config['noise_level'], f'example/figs/single_pen_FFN_fig_{config["noise_level"]}.png')
